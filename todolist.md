...
- [x] Nara Intro Screen VRAM 緩釋：釋放序列貼圖避免常駐佔用（負責：Codex）
- [x] Nara Intro Screen VRAM 測試：執行 .\gradlew.bat build 並確認釋放流程（負責：Codex）
- [x] Nara Init Screen VRAM 緩釋：釋放背景緩存貼圖避免常駐佔用（負責：Codex）
- [x] Nara Init Screen VRAM 測試：執行 .\gradlew.bat build 並確認釋放流程（負責：Codex）
