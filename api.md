...
## 版本資訊
- **文件版本**：0.1.23
- **最後更新**：記錄儀式結構 JEI 投影介面與藍圖管理，對外 API 無變動
...
- **0.1.23**：新增 Ritual 結構投影 JEI 互動需求與藍圖管理介面說明，確認仍無 REST 端點。
- **0.1.22**：補充儀式多基座（ring1/2/3）結構統計與文檔說明，確認仍無 REST 端點。
- **0.1.21**：記錄儀式結構統計鍵命名一致性與 Ritual JEI 視覺化計畫，確認仍無 REST 端點。
- **0.1.19**：記錄符文石外觀／碰撞盒調整與模型資料生成方式，確認仍無 REST 端點。
- **0.1.18**：新增儀式結構驗證輸出（基座朝向、粉筆顏色／圖案統計）與粉筆語系轉換說明，確認仍無 REST 端點。
- **0.1.17**：完成儀式結構驗證、基座同步與符文快取調整，確認仍無 REST 端點。
- **0.1.20**：記錄儀式啟動需先通過結構驗證（避免誤扣催化劑），確認仍無 REST 端點。

- **0.1.16**：紀錄儀式驗證 Validator 架構與序列化整併，確認仍無 REST 端點。

- **0.1.15**：整理 Ritual block / blockentity 類別並更新內部文件索引，確認仍無 REST 端點。
- **0.1.14**：紀錄共鳴水晶物品模型縮放與專用材質調整，確認仍無 REST 端點。

- **0.1.14**：記錄 Nara UI 序列貼圖與背景緩存策略，確認仍無 REST 端點。

- **0.1.13**：補充粉筆符號與符文石顯示名稱翻譯，確認仍無 REST 端點。
- **0.1.13**：記錄 Ritual block / blockentity 類別整理與對應文件位置，對 REST 端點無影響。
- **0.1.12**：新增導管錯誤提示與太陽能升級畫面語系鍵，確認仍無 REST 端點。
